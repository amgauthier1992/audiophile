...
const HeadphonesProduct: React.FC<{ params: { productId: string } }> = ({ params }) => {
  ...
  return (
    <>
      ...
      <section
        className={`${headphones?.slug} flex flex-col items-center gap-y-16 px-6 md:gap-y-20 md:px-10 lg:gap-y-32 lg:px-20 xl:px-40 2xl:px-56 3xl:px-60 4xl:px-80`}
      >
        <div className='product-card flex w-full flex-col md:flex-row'>
          <img
            className='product-img md:hidden'
            src={headphones?.image.mobile}
            alt={headphones?.name}
          />
          <img
            className='product-img xs:hidden md:block md:w-[281px] lg:hidden'
            src={headphones?.image.tablet}
            alt={headphones?.name}
          />
          <img
          ...
          <div className='product-summary flex flex-col gap-y-4 md:justify-center'>
            ...
            <h2 className='text-2xl font-bold uppercase md:text-3xl'>{headphones?.name}</h2>
            <p className='text-[#10101080]'>{headphones?.description}</p>
            <p className='text-[18px] font-bold tracking-[1.29px]'>
              {formatCurrency(Number(headphones?.price))}
            </p>
        ...
        <div className='flex flex-col lg:flex-row'>
          ...
          <div className='in-the-box flex w-full flex-col gap-y-4 pt-8 md:flex-row md:justify-between md:gap-x-40 lg:mt-[-78px] lg:flex-col lg:items-center lg:justify-start lg:pt-0'>
            ...
            <div className='mr-auto flex flex-col gap-y-2 lg:mr-0'>
              {headphones?.includes.map((product) => (
                <p
                  key={product.item}
                  className='text-[#10101080]'
                >
        ...
        <div className='product-gallery flex flex-col gap-y-4 3xl:w-full'>
          <img
            ...
            src={headphones?.gallery.first.mobile}
            alt={headphones?.name}
          />
          <img
            className='rounded-md md:hidden'
            src={headphones?.gallery.second.mobile}
            alt={headphones?.name}
          />
          <img
            className='rounded-md md:hidden'
            src={headphones?.gallery.third.mobile}
            alt={headphones?.name}
          />
          <div className='flex flex-row gap-x-4'>
            <div className='flex flex-col justify-between'>
              <img
                ...
                src={headphones?.gallery.first.tablet}
                alt={headphones?.name}
              />
              <img
                className='rounded-md xs:hidden md:block lg:hidden'
                src={headphones?.gallery.second.tablet}
                alt={headphones?.name}
              />
            </div>
            <div>
              <img
                className='rounded-md xs:hidden md:block lg:hidden'
                src={headphones?.gallery.third.tablet}
                alt={headphones?.name}
              />
            </div>
          </div>
