...
const SpeakersProduct: React.FC<{ params: { productId: string } }> = ({ params }) => {
  ...
  return (
    <>
      ...
      <section
        className={`${speakers?.slug} flex flex-col items-center gap-y-16 px-6 md:gap-y-20 md:px-10 lg:gap-y-32 lg:px-20 xl:px-40 2xl:px-56 3xl:px-60 4xl:px-80`}
      >
        <div className='product-card flex w-full flex-col md:flex-row'>
          <img
            className='product-img md:hidden'
            src={speakers?.image.mobile}
            alt={speakers?.name}
          />
          <img
            className='product-img xs:hidden md:block md:w-[281px] lg:hidden'
            src={speakers?.image.tablet}
            alt={speakers?.name}
          />
          <img
          ...
          <div className='product-summary flex flex-col gap-y-4 md:justify-center'>
            ...
            <h2 className='text-2xl font-bold uppercase md:text-3xl'>{speakers?.name}</h2>
            <p className='text-[#10101080]'>{speakers?.description}</p>
            <p className='text-[18px] font-bold tracking-[1.29px]'>
              {formatCurrency(Number(speakers?.price))}
            </p>
            <div className='product-controls flex flex-row justify-between sm:justify-start sm:gap-x-6'>
              ...
              <Button
                cta='Add to Cart'
                onClick={(e) => speakers && handleAddToCart(e, speakers)}
                variant='primary'
              />
            </div>
          </div>
        ...
        <div className='product-gallery flex flex-col gap-y-4 3xl:w-full'>
          ...
          <img
            className='rounded-md md:hidden'
            src={speakers?.gallery.second.mobile}
            alt={speakers?.name}
          />
          <img
            className='rounded-md md:hidden'
            src={speakers?.gallery.third.mobile}
            alt={speakers?.name}
          />
          <div className='flex flex-row gap-x-4'>
            <div className='flex flex-col justify-between'>
              ...
              <img
                className='rounded-md xs:hidden md:block lg:hidden'
                src={speakers?.gallery.second.tablet}
                alt={speakers?.name}
              />
            </div>
            <div>
              <img
                className='rounded-md xs:hidden md:block lg:hidden'
                src={speakers?.gallery.third.tablet}
                alt={speakers?.name}
              />
            </div>
          </div>
